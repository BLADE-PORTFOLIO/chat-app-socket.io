...
const updateGroup = async (req, res) => {
  try {
    ...
    if (req.body.files !== undefined && req.body.files.length > 0) {
      const fileLocations = await convertBlobToBlobArrayAndSave(req.body.filename, await convertDataUrlsToBlob(req.body.files), 'images');
      updateObj = {
        name: req.body.name,
        image: fileLocations[0],
        limit: req.body.limit,
      }
      const oldGroup = await Group.findById(req.body.id);
      const temp = __dirname.split('\\');
      temp.pop();
      const dir = temp.join('\\');
      fs.unlinkSync(dir + '\\public\\' + oldGroup.image);
    } else {
      updateObj = {
        name: req.body.name,
    ...
    const data = await Group.findByIdAndUpdate(req.body.id, {
      $set: updateObj,
    });
    console.log(data);
...
//   try {
//     const group = await Group.findById(req.body.id);
//     return res.status(400).json({ success: true, id: group.room });
//   } catch (error) {
//     return res.status(400).json({ success: false, msg: error.message });
//   }
// }
